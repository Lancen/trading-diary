...
**禁止 `git add -A`、`git add .`、`git add *`。每次 commit 前必须按文件路径逐一暂存。**

- `git add <具体文件>` 逐个暂存，每个文件都经过确认
- `git commit -a` 同样禁止——它等价于 `git add -A`
- commit 前用 `git diff --stat` 检查变更范围
- 发现陌生文件（未创建、未讨论）立即排查来源
- agent 产出的文件必须逐项审查后再加入暂存

**为什么**: `git add -A` 会将临时研究文件、日志、图片、沙箱文档等一次性吞入 repo。逐文件暂存是最后一道防线——即使文件被 agent 意外创建，也不会滑入版本历史。
...
**任何一项为 ❌ → BLOCKED → 必须先补完审查 → 才能问"继续？"**。

- 审查结论必须来自独立 subagent（不能是自己读代码判断）
- 审查 agent 的 prompt MUST 包含 design doc 路径
- 审查发现的问题必须在进入下一批前修复并重新审查
