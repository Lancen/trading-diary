...
### A7. Token 效率：读文件先看结构
**需要了解文件内容时，优先用 offset+limit 分段读取，禁止一次性全量读取 200+ 行的大文件。** 先读前 50 行了解类结构/导入，再按需精读目标方法区域。
...
### A9. 构建性能：控制内存与并发
**避免连续高频构建导致系统内存耗尽。**
- Gradle 测试：**禁止 `--no-daemon`**，复用 Daemon 进程避免重复启动 JVM
- 前端验证：优先用 `npx tsc --noEmit` 做类型检查，仅在最终提交前跑 `next build`
- 后端验证：优先用 `./gradlew compileJava` 做编译检查，测试仅在实现完成后跑一次
- **禁止连续 3 次以上构建命令而不间隔等待**，两次构建间至少留 5 秒
- 全量测试 (`./gradlew test`) 和全量构建 (`next build`) 同一时刻只跑一个
...
| 规则 | 文件 | 核心要点 |
|------|------|----------|
| 分层+数据访问 | [backend-layering-data-access.md](.claude/rules/backend-layering-data-access.md) | Controller→Service→Mapper 不可逆、XML 优先 `<choose>` 白名单、UpsertHelper/JsonNodeHelper/RetryPolicy/FetchResult |
| Service 接口+实现类 | [service-interface-impl.md](.claude/rules/service-interface-impl.md) | 接口在 `service/`，实现类在 `service/impl/`，注入接口类型 |
| 批量写入 | [batch-db-write.md](.claude/rules/batch-db-write.md) | 循环写入必须用 `BatchSqlRunner`，批量大小用常量 |
| 采集管线 | [collection-pipeline.md](.claude/rules/collection-pipeline.md) | 采集类型分级、日历路由、启动要求 |
| 代码注释 | [code-comment-standards.md](.claude/rules/code-comment-standards.md) | 中文 Javadoc、行内注释、XML mapper 注释、测试注释 |
| 变更态势感知 | [change-sense.md](.claude/rules/change-sense.md) | 需求模糊度/架构影响度/风险等级 评分 |

### 前端

| 规则 | 文件 | 核心要点 |
|------|------|----------|
| 前端规范 | [frontend-conventions.md](.claude/rules/frontend-conventions.md) | React Query 统一数据获取、类型/格式化集中、参数化组件去近克隆、Badge 内联 |
| 前端组件目录 | 同上 | `components/` 业务组件、`components/chart/` 图表、`components/ui/` shadcn |
