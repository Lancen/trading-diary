...
import type { SectorItem, PaginatedData } from "@/lib/types";
import { fmtYi, isStale, fmtDate } from "@/lib/format";
...
export interface SectorTypeConfig {
  ...
  countLabel: string;
  basePath: string;
  stockFilterKey: string;
  marginStatsPath: string;
  externalLink: (code: string) => string;
}

type SortField = "stockCount" | "marginBalance" | "marginChange" | "shortBalance" | "shortChange" | "volumePct";

const SORT_FIELDS: { key: SortField; label: string }[] = [
  { key: "stockCount", label: "股票数" },
...
export default function SectorListPage({ config }: { config: SectorTypeConfig }) {
  ...
  useEffect(() => {
    const pollMap = pollRef.current;
    const timerMap = revertTimers.current;
    return () => {
      pollMap.forEach(v => clearInterval(v));
      timerMap.forEach(v => clearTimeout(v));
    };
  }, []);

  const queryParams: Record<string, string | number> = { sortBy, sortDir, page, size: 50 };
  if (keyword) queryParams.keyword = keyword;

  const queryKey = keys.sectors(config.sectorType, queryParams);

  const { data: result, isLoading, refetch } = useApiQuery<PaginatedData<SectorItem>>(
    queryKey,
    config.apiPath,
    queryParams,
  );
  ...
  const invalidateSectors = () =>
    queryClient.invalidateQueries({ queryKey: ["sectors", config.sectorType] });
  ...
  const handleScrape = async (code: string) => {
    ...
    try {
      await api.post("api/v1/admin/market/constituents/scrape", { searchParams: { type: config.sectorType, code } }).json();
      const timer = setInterval(async () => {
        try {
          const res = await api.get("api/v1/admin/market/constituents/scrape/status", { searchParams: { type: config.sectorType, code } }).json<{ code: number; data: { status: string; snapDate?: string; relationCount?: number; error?: string } }>();
  ...
  const handlePin = async (code: string, pinned: boolean) => {
    try {
      await api.post("api/v1/admin/market/pin", { json: { type: config.sectorType, code, pinned: String(pinned) } }).json();
      invalidateSectors();
  ...
  const handleMovePin = async (code: string, direction: "up" | "down") => {
    ...
    try {
      await api.post("api/v1/admin/market/pin/reorder", { json: { type: config.sectorType, codes } }).json();
      invalidateSectors();
    } catch (e) { console.error(e); }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-3 items-end">
        <div><label className="block text-xs text-gray-500 mb-1">{config.searchLabel}</label><input value={keyword} onChange={(e) => { setKeyword(e.target.value); setPage(1); }} placeholder="输入搜索" className="rounded-lg border px-3 py-2 text-sm w-40" /></div>
      ...
      <div className="overflow-x-auto rounded-lg border">
        <table className="w-full text-sm min-w-[1000px]">
          ...
          <tbody>
            {isLoading ? <tr><td colSpan={12} className="px-4 py-8 text-center text-gray-400">加载中...</td></tr> :
             ...
             data.map((item) => {
              ...
              <tr key={item.code} className={`border-b hover:bg-gray-50${item.pinned ? " bg-blue-50" : stale ? " bg-amber-50" : ""}`}>
                ...
                <td className="px-3 py-2 text-blue-600 cursor-pointer hover:underline" onClick={() => router.push(`${config.basePath}/${item.code}?name=${encodeURIComponent(item.name)}`)}>
                ...
                <td className="px-3 py-2 text-xs">
                  <a href={config.externalLink(item.code)} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">同花顺</a>
                </td>
                ...
                <td className="px-3 py-2 text-center text-blue-600 cursor-pointer underline" onClick={() => router.push(`/admin/stocks?${config.stockFilterKey}=${item.name}`)}>{item.stockCount}</td>
                <td className="px-3 py-2 text-right text-blue-600 cursor-pointer" onClick={() => router.push(`${config.marginStatsPath}/${item.code}`)}>{fmtYi(item.marginBalance)}</td>
                <td className={`px-3 py-2 text-right cursor-pointer ${item.marginChange > 0 ? "text-red-600" : "text-green-600"}`} onClick={() => router.push(`${config.marginStatsPath}/${item.code}`)}>{item.marginChange > 0 ? "↑" : "↓"}{fmtYi(Math.abs(item.marginChange))}</td>
                <td className="px-3 py-2 text-right cursor-pointer" onClick={() => router.push(`${config.marginStatsPath}/${item.code}`)}>{fmtYi(item.shortBalance)}</td>
                <td className={`px-3 py-2 text-right cursor-pointer ${item.shortChange > 0 ? "text-red-600" : "text-green-600"}`} onClick={() => router.push(`${config.marginStatsPath}/${item.code}`)}>{item.shortChange > 0 ? "↑" : "↓"}{fmtYi(Math.abs(item.shortChange))}</td>
      ...
      <div className="flex items-center justify-between text-sm text-gray-500">
        <span>共 {total} {config.countLabel}</span>
