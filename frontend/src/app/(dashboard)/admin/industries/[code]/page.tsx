...
export default function IndustryDetailPage() {
  ...
  const { data: detailData } = useApiQuery<SectorStockItem[]>(
    ["admin", "market", "industries", code],
    `api/v1/admin/market/industries/${code}/stocks`
  );

  const { data: klineData, isLoading: klineLoading } = useApiQuery<SectorIndexDaily[]>(
    ["admin", "sector-index-daily", "industries", code, range],
    `api/v1/admin/sector-index-daily/industries/${code}`,
    { startDate }
  );

  const { data: marginData, isLoading: marginLoading } = useApiQuery<SectorMarginDaily[]>(
    ["admin", "sector-margin", "industries", code, range],
    `api/v1/admin/sector-margin/industries/${code}`,
    { startDate }
  );

  const { data: marginDiffData } = useApiQuery<SectorMarginDiff>(
    ["admin", "market", "industries", code, "margin-diff"],
    `api/v1/admin/market/industries/${code}/margin-diff`,
    { startDate }
  );

  const { data: correlationData } = useApiQuery<{ sectors: { code: string; name: string; score: number }[] }>(
    ["admin", "market", "industries", code, "correlations"],
    `api/v1/admin/market/industries/${code}/correlations`
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <a href="/admin/industries" className="text-sm text-gray-400 hover:text-blue-600">← 行业板块</a>
      </div>

      <h1 className="text-2xl font-bold">行业详情 {code}</h1>
