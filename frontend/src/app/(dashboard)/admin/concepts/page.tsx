  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">概念板块</h1>
          <p className="text-sm text-gray-500">浏览已采集的概念板块数据</p>
        </div>
        ...
        <table className="w-full text-sm min-w-[1000px]">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr key={item.code} className={`border-b hover:bg-gray-50 cursor-pointer ${item.pinned ? "bg-yellow-50/50" : ""}`} onClick={() => router.push(`/admin/concepts/${item.code}`)}>
                ...
                <td className="px-3 py-2">
                  <button className="text-xs text-blue-600 hover:underline" onClick={(e) => { e.stopPropagation(); router.push(`/admin/concepts/${item.code}`); }}>详情</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="text-sm text-gray-500">共 {items.length} 个概念板块</div>
    </div>
  );
}
