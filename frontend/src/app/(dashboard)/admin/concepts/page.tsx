  ...
  const handleScrape = async (code: string) => {
    ...
    try {
      ...
      const timer = setInterval(async () => {
        try {
          const res = await api.get("api/v1/admin/market/constituents/scrape/status", { searchParams: { type: "concept", code } }).json<{ code: number; data: { status: string; snapDate?: string; relationCount?: number; error?: string } }>();
            ...
            {loading ? <tr><td colSpan={12} className="px-4 py-8 text-center text-gray-400">加载中...</td></tr> :
             ...
             data.map((item) => {
              ...
              <tr key={item.code} className={`border-b hover:bg-gray-50${item.pinned ? " bg-blue-50" : stale ? " bg-amber-50" : ""}`}>
                ...
                <td className="px-3 py-2 text-blue-600 cursor-pointer hover:underline" onClick={() => router.push(`/admin/concepts/${item.code}?name=${encodeURIComponent(item.name)}`)}>
