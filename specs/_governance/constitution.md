...
宪法只定义 **What（做什么、为什么）**，不定义 **How（怎么做）**。边界清晰，宪法才不会膨胀成开发手册。判断标准：**这条规则在回答"我们追求什么"，还是"用什么工具/参数实现"？**

| 维度 | 宪法（What） | 技术规范（How） |
|------|-------------|-----------------|
| 价值排序 | 类型安全 > 开发速度 | ESLint 规则集选哪个 |
| 质量底线 | 不可测试的代码视为不完整 | 覆盖率阈值的具体数字 |
| 架构约束 | 分层不可逆 | 每层具体有几个子包 |
| 领域规则 | 金额使用 BigDecimal | `decimal(20,8)` 精度值 |
| 安全原则 | 密码 MUST 加盐哈希存储 | BCrypt cost 具体数值 |
| 一致性标准 | API 响应使用统一格式 | 分页参数命名为 page/size |

> 本文档中代码示例仅用于说明原则意图，具体实现参数以 `docs/standards/technical-standards.md` 为准。
...
> **裁决**：当"快速直连 Mapper"与分层架构冲突时，以分层架构为准。
...
> **裁决**：当直觉优化与先写正确代码冲突时，先写正确代码，用数据说话。
...
**Version**: 0.1.0 | **Ratified**: 2026-05-09 | **Last Amended**: 2026-05-11
