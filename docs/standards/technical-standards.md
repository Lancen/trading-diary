...
| 版本 | 日期 | 变更 |
|------|------|------|
| 1.0.0 | 2026-05-09 | 从宪法 v2.0.0 中拆分技术规范章节，新增阶段标记 |
| 1.1.0 | 2026-05-09 | ORM 改为 MyBatis-Plus；补充实体规范、软删除、API 限流、错误码模块编号；Swagger 关闭移至 Phase 2；修复 Lombok `@Data` 规则；修正事务查询传播级别 |
| 1.2.0 | 2026-05-09 | 统一表 COMMENT 格式（含业务场景和生命周期）；新增 §2.4 数据库设计文档（ERD、生命周期说明）；新增 COMMENT/生命周期/限流规则；§8 测试规范拆分补充；§7 新增日志 JSON 示例 |
| 1.2.1 | 2026-05-09 | 时区传递机制改为请求头 `X-Timezone`；宪法领域约束 §2 和技术规范 §3.2 同步更新 |
| 1.3.0 | 2026-05-09 | 新增前端命名约定；新增 Flyway 迁移脚本命名约定（§4.2）；429 限流响应体补充完整 JSON 格式；宪法 Phase 0 检查清单补充 `X-Timezone` 项 |
| 1.3.1 | 2026-05-09 | 分页请求参数统一 `page`/`size`，MP 配置映射；异常响应格式统一 `ApiResponse<T>`（data=null）；CLAUDE.md 补充 @MybatisPlusTest |
