# 阿里技术 Agent Skill 规范、构建与设计范式

> 来源：阿里妹导读 - 淘天集团-淘宝平台事业部-客户运营部技术实践总结
> 原文链接：https://mp.weixin.qq.com/s/LCpiLyLnRn5WyuHpribyHw
...
**核心定义**：Skill 是一种可复用的 Prompt 增强包，通过渐进式加载机制为 Agent 注入领域知识和工作流程。

**关键数据**：
- 2025年12月 Anthropic 发布 Skill 规范作为开放标准
- 已被 **33+个 Agent 产品** 采纳（Claude Code、OpenAI Codex、GitHub Copilot、VS Code、Cursor、Gemini CLI、Kiro 等）
...
在 SKILL.md 中引用其他文件时，请使用相对于 Skill 根目录的路径：
...
## 二、Skill-Creator 核心思想（Anthropic 官方）

### 2.1 设计哲学

**核心思想**：像做机器学习一样做 Prompt Engineering —— 有训练集、测试集、评估指标、迭代优化循环、防过拟合机制。
...
Skill 要被使用无数次、面对无数种 prompt。如果只为测试用例做针对性修改，skill 就废了。遇到顽固问题，尝试换个隐喻或推荐不同的工作模式，而不是加更多死板约束。

#### 2. 解释"为什么"而非堆砌"必须"

这是全文最核心的洞察。今天的 LLM 有良好的心智理论，与其写满大写的 ALWAYS 和 NEVER，不如解释清楚为什么某件事重要。
...
GitHub Issue #514（2026-03-04）：

> "A single description optimization run with 20 eval queries (3 runs each = 60 sessions) consumed ~69% of a 5-hour time block, with 0 actionable results."

**问题根源**：当用户使用 Opus 会话时，description 优化会启动 60+ 个 Opus 级别的 `claude -p` 子进程，而触发检测本质上只是一个"是/否"的二元信号。
...
对于简单的 Skill，这套流程的开销远超 Skill 本身的价值。
...
以 3 个测试用例为例，单轮评测就会产生 10 个子 Agent。

**问题四：Description 优化对"操作型 Skill"效果有限**

对于某些"操作型"Skill（如"运行部署脚本"、"生成日报"），Claude 本身就能直接处理，不会主动去查询 Skill，导致触发率始终为 0%。

**问题五：Skill 膨胀风险**

随着迭代改进，Skill 有膨胀倾向——每次改进都可能增加新的指令、示例、边界情况处理，最终导致 Skill 体积失控。
...
## 三、Writing-Skills 核心思想（Superpowers 框架）
...
框架包含 14 个可组合的 Skill，覆盖从头脑风暴到代码交付的完整开发流程。
...
## 四、Skill 设计模式（Google ADK）

> 来源：Google Cloud Tech

规范告诉我们"Skill 什么样"，但没告诉我们"Skill 内部的逻辑该怎么设计"。Google ADK 团队总结了 5 种反复出现的设计模式。
...
| Anthropic 黑客马拉松获胜者的完整 Claude Code 配置集合 | https://github.com/affaan-m/everything-claude-code |
| 开源 skills 市场 | https://skills.sh, https://skillsmp.com, https://github.com/openclaw/clawhub |
