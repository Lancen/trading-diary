...
│ 7. 执行 ── 循环审查门禁                                                                   │
│                                                                                          │
│  tasks.md 按 Phase 拆批 → 每批:                                                           │
│                                                                                          │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                  │
│  │ 派发实现 agent │→│ 加载 TDD skill│→│ Spec 合规审查  │→│ 代码质量审查   │──→ 进入下一批     │
│  │ implementer  │  │RED→GREEN→REF│  │ spec-reviewer│  │code-reviewer │                  │
│  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘                  │
│         ↑                                      │                   │                       │
│         └────────── 修复 ──────────────────────┴─── 修复 ──────────┘                       │
│                                                                                          │
...
**analyze MUST 额外检查 design doc 的技术决策是否都有对应任务**：逐条对照 design doc "功能描述"之前章节的技术方案，确认每个技术决策在 tasks.md 中有对应条目。设计有两层策略（日常增量 + 全量回填）但 tasks.md 只有一个模糊任务的，必须拆分。
...
| **模型选择** | **派发时 MUST 显式传 `model` 参数**。机械任务用 haiku（flash），审查任务用 opus（pro）。判断标准见下表 |
...
**适用范围**：所有实现类任务（Service、Controller、核心逻辑）
...
| design doc 多层策略在 tasks.md 合成一个模糊任务 | agent 做完了 task 字面描述（底层组件）但遗漏端到端入口（上层调度） | analyze 阶段对照 design doc 逐条检查，端到端能力链（入口→调度→底层调用）必须完整 |
