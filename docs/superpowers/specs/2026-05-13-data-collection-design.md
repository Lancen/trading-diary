...
唯一键 `(stock_code, snap_date)`。数据来源：从 `stock_zh_a_spot_em` 返回字段中的两融标记过滤（实施时需验证实际字段名）。
...
```
1. trade_calendar → 2. stock_info ─┬→ margin_stock（从中过滤）
                                   └→ stock_daily（从中提取 OHLC）
                                   ↓
                   3. industry → stock_industry（与 concept 并行）
                   4. concept → stock_concept
                   5. margin_daily ← 依赖 trade_calendar + margin_stock
```
...
collectStockSpot()                             // 全 A 股快照（提取两融标的到 margin_stock）
...
| 两融标记字段名未知 | 实施时验证 `stock_zh_a_spot_em` 返回字段，必要时查阅 AKShare 源码 |
