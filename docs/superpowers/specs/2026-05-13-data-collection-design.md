...
| 集成方式 | AKTools Python 包（`pip3 install aktools`），Java HTTP 调用 | 1000+ 接口全覆盖 |
...
├── collection/                              # 新增包
│   ├── client/
│   │   └── AKToolsClient.java              # HTTP 客户端，统一封装对 AKTools 的调用
│   ├── controller/
...
统一封装对 AKTools 的 HTTP 调用（建议 Spring RestClient）。所有请求走同一个 base URL（`http://localhost:8081`），返回 JSON 字符串写入 `raw_data`。
...
开发和生产环境统一使用 Python `aktools` 包：

```bash
# 安装（一次性）
pip3 install aktools

# 启动（映射到 8081 端口，避免与后端 8080 冲突）
...
Python `aktools` 包自带 FastAPI + Uvicorn，提供完整的 AKShare HTTP API。

### 健康检查

Java 端在每次调用前可先访问 `GET /api/public/tool_trade_date_hist_sina`（最轻量接口）验证 AKTools 在线状态。连续失败时输出明确日志："AKTools 服务不可用"。
...
升级前在 dev 环境验证关键接口（`stock_zh_a_spot_em` / `stock_margin_detail_sse`），确认字段兼容后更新 prod。
...
| AKTools 服务 版本升级接口变化 | 锁定镜像版本，升级前先在 dev 验证 |
...
| AKTools 服务 不可用 | 连续失败时日志明确提示，管理后台红色标记 |
