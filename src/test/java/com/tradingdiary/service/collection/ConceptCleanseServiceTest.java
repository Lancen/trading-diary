...
import com.tradingdiary.service.collection.impl.ConceptCleanseServiceImpl;
...
class ConceptCleanseServiceTest {

    private static final String CONCEPT_CODE = "GN001";
    ...
    void setUp() {
        ...
        service = new ConceptCleanseServiceImpl(
    ...
    void shouldInsertNewStocksAndLogAdd() {
        List<StockConcept> dbRelations = Arrays.asList(
                buildDbRelation("000001"),
                buildDbRelation("000002")
        );
        when(stockConceptMapper.selectList(any())).thenReturn(dbRelations);

        String rawJson = jsonArray(jsonStock("000001"), jsonStock("000002"), jsonStock("000003"));

        int result = service.cleanseCons(rawJson, CONCEPT_CODE, SNAP_DATE);
        ...
        assertThat(log.getClassificationType()).isEqualTo("CONCEPT");
        assertThat(log.getSectorCode()).isEqualTo(CONCEPT_CODE);
    ...
    void shouldSoftDeleteRemovedStocksAndLogRemove() {
        StockConcept db1 = buildDbRelation("000001");
        StockConcept db2 = buildDbRelation("000002");
        StockConcept db3 = buildDbRelation("000003");
        List<StockConcept> dbRelations = Arrays.asList(db1, db2, db3);
        when(stockConceptMapper.selectList(any())).thenReturn(dbRelations);
    ...
    void shouldInsertAllWhenDbEmpty() {
        when(stockConceptMapper.selectList(any())).thenReturn(Collections.emptyList());
    ...
    // ──────────────────────────────────────────────
    // Test: Empty JSON — no constituents parsed
    // ──────────────────────────────────────────────
    ...
    private StockConcept buildDbRelation(String stockCode) {
        StockConcept sc = new StockConcept();
        sc.setId((long) stockCode.hashCode());
        sc.setStockCode(stockCode);
        sc.setConceptCode(CONCEPT_CODE);
        sc.setSnapDate(SNAP_DATE.minusDays(1));
